...
Plugin URI: http://wordpress.org/plugins/simpletick-eticket-widget/ 
...
class SimpleTix{
	...
	{		
		...
		{		
			...
			$targetPath		=	$upload_dir['basedir'].'/simpletick-eticket-widget/'.$file_name;						
	...
	function custom_uploadify_callback( )
	{
		...
		$upload_dir 	=	wp_upload_dir();
		$targetPath		=	$upload_dir['basedir'].'/simpletick-eticket-widget';
		...
		if (!empty($_FILES) && $_POST['token'] == $verifyToken)
		{
			...
			$file_url		=	$upload_dir['baseurl'].'/simpletick-eticket-widget/'.time().$_FILES['Filedata']['name'];
	...
	{			
		...
		wp_register_script( 'simpletix_hashchange', plugins_url( 'simpletick-eticket-widget/js/jquery.hashchange.min.js' ) );
		wp_enqueue_script( 'simpletix_hashchange' );
		
		wp_register_script( 'simpletix_easytab', plugins_url( 'simpletick-eticket-widget/js/jquery.easytabs.min.js' ) );
		wp_enqueue_script( 'simpletix_easytab' );		
		
		wp_register_script( 'simpletix_uploadify', plugins_url( 'simpletick-eticket-widget/uploadify/jquery.uploadify.min.js' ) );
		wp_enqueue_script( 'simpletix_uploadify' );		
		
		wp_register_style( 'simpletix_colorbox_css', plugins_url( 'simpletick-eticket-widget/css/colorbox.css' ) );
		wp_enqueue_style( 'simpletix_colorbox_css' );				
		
 		wp_register_style( 'simpletix_tabs', plugins_url( 'simpletick-eticket-widget/css/tabs.css' ) );
		wp_enqueue_style( 'simpletix_tabs' );				
		
		wp_register_style( 'simpletix_uploadify', plugins_url( 'simpletick-eticket-widget/uploadify/uploadify.css' ) );
		wp_enqueue_style( 'simpletix_uploadify' );				
		
		wp_register_style( 'simpletix_style', plugins_url( 'simpletick-eticket-widget/css/style.css' ) );
		wp_enqueue_style( 'simpletix_style' );						
		
		wp_register_style( 'simpletix_horizontal_tabs', plugins_url( 'simpletick-eticket-widget/css/horizontal-tabs.css' ) );
		...
		<div id="mm_sg_container" style="display:none;">			
			...
			<form id="simpletix-sg-form">
				...
				<div id="tab-container" class='tab-container'>
					...
					<img id="simpletix_loading_image" style="display:none;margin-left:200px;" src="<?php echo WP_PLUGIN_URL; ?>/simpletick-eticket-widget/images/loading1.gif" />
